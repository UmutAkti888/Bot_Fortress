...
# Paths now come from core.config — no more relative ".." calculations.
from core.config import PAPERS_DIR, RESULTS_FILE
import os
...
def search(
    ...
    # Wrap results with query metadata envelope
    wrapper = {
        "_query": {
            "keywords":    keywords,
            "from_year":   from_year,
...
def search_and_download(keywords: list[str], max_results: int = 10) -> list[dict]:
    """Convenience function: search then immediately download all PDFs."""

...
#
# Purpose: For each paper in the corpus, extract what problem it solves,
# what system it targets, and what approach it uses. Aggregate results into
# a frequency table showing what problems the field is actually trying to fix.
#
# This answers the professor's core question: "What problem does your work solve?"
# by grounding the contribution in documented, recurring, unsolved problems
# drawn from your own corpus rather than abstract novelty claims.
...
# System message: strict JSON output, no conversation.
SYSTEM_MESSAGE = (
...
# The per-batch extraction prompt.
# We ask for a JSON array with one object per paper.
EXTRACTION_PROMPT = """\
...
def _build_batch_prompt(batch: list[dict]) -> str:
    ...
    We include only title + abstract — enough for the LLM to extract the four fields.
    Abstract is capped at 300 chars to keep batch prompts short.
    """
    paper_block = ""
...
def _parse_json_response(text: str) -> list[dict]:
    ...
    # Second try: find the first [...] block using regex
    ...
    return []   # failed — batch will be marked as error, not crash
...
def extract_problems(
    ...
    """
    Run per-paper extraction across all papers in the corpus.
    Saves results to problem_map.json.

    Returns a summary dict with extraction stats and aggregated results.

    This function is designed to be called from the Flask route — it runs
    synchronously and returns when all batches are done.
    For live progress in the browser, use extract_problems_stream() instead.
    """
    papers = _read_merged_papers()
    if not papers:
        return {"error": "No merged results found. Run a search and merge first."}
    ...
    for i, batch in enumerate(batches):
        print(f"[ProblemMap] Batch {i+1}/{len(batches)} — {len(batch)} papers...")
        ...
        try:
            ...
            if parsed:
                ...
                # JSON parse failed — record failures with original titles
                errors += len(batch)
                for p in batch:
                    extractions.append({
                        "title":           p.get("title", ""),
            ...
            print(f"[ProblemMap] Batch {i+1} error: {e}")
            errors += len(batch)

    # Aggregate: count problem keywords and target systems
    ...
    print(f"[ProblemMap] Done. {len(extractions)} extracted, {errors} errors.")
    return result
...
def extract_problems_stream(
    ...
    """
    Generator version of extract_problems — yields progress events for SSE.
    ...
    for i, batch in enumerate(batches):
        # Send progress update before processing each batch
        yield json.dumps({
            "type":      "progress",
            "batch":     i + 1,
            "total":     len(batches),
...
def _aggregate(extractions: list[dict]) -> dict:
    ...
    Returns top-N lists for display in the UI.

    We split each multi-word field into phrases and count occurrences of
    common domain keywords to group related problems together.
    """
    ...
    # Keywords we care about — more specific than single words
    PROBLEM_KEYWORDS = [
    ...
    for entry in extractions:
        ...
        # For approaches, count mentions of key methods
